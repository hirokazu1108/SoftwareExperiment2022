...
/* ??????????????????鐃???????? */
int xBegin            = 0;    /* ????????鐃???????????????????鐃????????X膓削申??? */
int yBegin            = 0;    /* ????????鐃???????????????????鐃????????Y膓削申??? */
int PressButton       = 0;    /* ?????????????????????????????????鐃????????鐃?????鐃??????1:鐃?????,2:鐃?????,3:?????? */
float CameraAzimuth   = 90.0; /* ?????鐃?????????鐃???????????????????????????? */
float CameraElevation = 0.0;  /* ?????鐃?????????鐃?????????????鐃????????????? */
float CameraDistance  = 5.0;  /* ?????鐃?????????鐃??????????????????鐃??????????????????????? */
float CameraX         = 0;  /* ?????鐃?????????鐃?????????????X膓削申??????? */
float CameraY         = 1.0;  /* ?????鐃?????????鐃?????????????Y膓削申??????? */
float CameraZ         = 0.0;  /* ?????鐃?????????鐃?????????????Z膓削申??????? */
float BoxX[3];                /* ???鐃??????X膓削申??? */
float BoxVx[3];               /* ???鐃??????X?????????鐃順蓋?????鐃????? */
float BoxY[3];
float BoxZ[3];
char left  = 0;
char right = 0;
...
bool key8 = false;
bool startflag = true;
int bullet_Num = 0;   // ????????????????膓ｏ申??????????
bool can_attack = true;     // 膓ｏ申???????????????????鐃?????????true
...
int joyconev();
void move(void);
void create_bullet(int num);    // 膓ｏ申??????????
void draw_bullet(int num);  // 膓ｏ申??????????
void move_bullet(int num);    // 膓ｏ申??鐃?????????????
void add_lifetime(int add_lifetimeID);  // 膓ｏ申????????????????鐃????????????????????
void count_time(int count_timeID);      // ??????????????鐃??????????????????????????????????????????????????鐃????????????
void del_bullet(void);  // 膓ｏ申????鐃?????????
void deleteBullet(int index);
void interval_attack(int interval_attackID);    // 膓ｏ申??????????????????????????????????????
...
int main(int argc, char **argv)
{
  ...
  default:
    ...
    myInit(game.clientName[clientID]);       /* ?????????鐃????????????????????????????鐃緒申??鐃????????????????? */
...
void display(void)
{
    ...
    /* ????????? */
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); /* ???????????鐃????????? */
    glMatrixMode(GL_MODELVIEW);                         /* 鐃??????????????????鐃????????????????????????????????????????????? */
    glLoadIdentity();                                   /* 鐃??????????????????????????? */

    /* ??????鐃?????????????? */
    // glMatrixMode(GL_PROJECTION);
    
    gluLookAt(CameraX, CameraY, CameraZ, /* ?????鐃?????????鐃????????? */
        player[clientID].pos.x, player[clientID].pos.y,player[clientID].pos.z,        /* 鐃???????????鐃??????鐃????????? */
        0, 0.5*cos(player[clientID].turn2), 0.0);         /* ?????鐃??????鐃??????????????????????????? */
    /*if(startflag == true){
        glPushMatrix();
        glColor3f(1.0, 0.0, 0);
   ...
   \
    ...
    glRotatef(90, 0, 1.0, 0);          /* Y??????BoxRotate(??)??鐃???? */
    ...
    move_bullet(bullet_Num);    // 膓ｏ申??????鐃順蓋??
    draw_bullet(bullet_Num);    // 膓ｏ申??????????
...
void myInit(char *windowTitle)
{
    ...
    /* OpenGL?????????鐃?????????鐃??????????????????????????? */
    glutInitWindowPosition(0, 0);            /* ?????????鐃????????????????????????? */
    glutInitWindowSize(winWidth, winHeight); /* ?????????鐃???????????鐃?????????? */
    // glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH);   /* ?????鐃?????????????? */

    glutCreateWindow(windowTitle);                      /* ?????????鐃???????????????????? */
    ...
    glutDisplayFunc(display);    /* ????????鐃緒申?????? */
    glutTimerFunc(15, timer, 0); /* ??鐃?????????????????15??????????鐃???????????????? */

    /* CG???????鐃緒申??鐃????? */
    glMatrixMode(GL_PROJECTION);             /* ?????????鐃?????(??????????膓究ｩ??)??????????????????????????????鐃?????? */
    glLoadIdentity();                        /* ?????????膓究何????????????????? */
    gluPerspective(45.0, aspect, 1.0, 100.0); /* ?????????膓究何???????????????? */
                                             /* ???????????45鐃?????, ????鐃?????鐃????? aspect?????????鐃????????????????????鐃????????? 1.0????????????????????????????鐃????????? 20.0 */
    glEnable(GL_DEPTH_TEST);                 /* ??????????????????????鐃???????????? */
    glClearColor(0.658, 0.88, 0.925, 1.0);     
