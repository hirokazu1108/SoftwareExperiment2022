...
int scoreBallNum = 0;
bool firstRecvPlayerInfo = false; //�ǡ����������ä����ɤ���
...
void joystick(unsigned int buttonMask, int x, int y, int z)
{
        ...
        switch (buttonMask) {
               ...
               break;
                ...
                break;
                case GLUT_JOYSTICK_BUTTON_E:/*5????????��??*/
                 key7 = true;
                break;
                case GLUT_JOYSTICK_BUTTON_F:/*6????????��??*/
                 key7 = true;
                break;
                case GLUT_JOYSTICK_BUTTON_G:/*5????????��??*/
                can_special = true;
                    useSpecial();
                break;
                case GLUT_JOYSTICK_BUTTON_H:/*5????????��??*/
                can_special = true;
                    useSpecial();
                break;
...
void myInit(char *windowTitle)
{
    ...
    glLoadIdentity();                        /* ???????????????ç????ä????????????????????? */
    gluPerspective(45.0, aspect, 1.0, 200.0); /* ???????????????ç????ä???????????????????? */
