...
int scoreBallNum = 0;
bool firstRecvPlayerInfo = false; //??????ï¿¥ç?³é??ç·???³é??ç·???³é??ç·???³é??ç·???³é????£ã????³é??ç·???³é??å®¿ã????³é??ç·????
int endFlag = 1; //1:å·???????????????????????-1:????????????????????????æ·???????  0:????????????????è¶???????????å±?å·½ç????è¾?????
...
/* ??????????????????è¾????????????? */
int xBegin            = 0;    /* ????????è¾????????????????????????è¾?????????????X?????????????å·???????? */
int yBegin            = 0;    /* ????????è¾????????????????????????è¾?????????????Y?????????????å·???????? */
int PressButton       = 0;    /* ?????????????????????????????????è¾?????????????è¾??????????è¾???????????1:è¾??????????,2:è¾??????????,3:?????? */
float CameraAzimuth   = 90.0; /* ?????è¾??????????????è¾????????????????????????????????? */
float CameraElevation = 0.0;  /* ?????è¾??????????????è¾??????????????????è¾?????????????????? */
float CameraDistance  = 5.0;  /* ?????è¾??????????????è¾???????????????????????è¾???????????????????????????? */
float CameraX         = 0;  /* ?????è¾??????????????è¾??????????????????X?????????????å·???????????? */
float CameraY         = 1.0;  /* ?????è¾??????????????è¾??????????????????Y?????????????å·???????????? */
float CameraZ         = 0.0;  /* ?????è¾??????????????è¾??????????????????Z?????????????å·???????????? */
float BoxX[3];                /* ???è¾???????????X?????????????å·???????? */
float BoxVx[3];               /* ???è¾???????????X?????????è¾?????è¾????????????????è¾?????????? */
float BoxY[3];
float BoxZ[3];
char left  = 0;
char right = 0;
...
bool startflag = true;
int bullet_Num = 0;   // ??????????????????????????????ç·???????????????????
bool can_attack = true;     // ??????????????ç·????????????????????????????è¾??????????????true
bool can_special = false;
int cnt_special = 0;
bool cameraflag = false;
double turn4;
double turn5;
float CameraX2         = 0;  /* ?????è¾??????????????è¾??????????????????X?????????????å·???????????? */
float CameraY2         = 1.0;  /* ?????è¾??????????????è¾??????????????????Y?????????????å·???????????? */
float CameraZ2         = 0.0; 
...
int joyconev();
void move(void);
void create_bullet(int num);    // ??????????????ç·???????????????????
void draw_bullet(int num);  // ??????????????ç·???????????????????
void move_bullet(int num);    // ??????????????ç·???????????è¾??????????????????
void add_lifetime(int add_lifetimeID);  // ??????????????ç·?????????????????????????è¾?????????????????????????
void count_time(int count_timeID);      // ??????????????è¾???????????????????????????????????????????????????????è¾?????????????????
void del_bullet(void);  // ??????????????ç·?????????????è¾??????????????
void deleteBullet(int index);
void interval_attack(int interval_attackID);    // ??????????????ç·???????????????????????????????????????????????
...
int main(int argc, char **argv)
{
  ...
  default:
    ...
    myInit(game.clientName[clientID]);       /* ?????????è¾?????????????????????????????????è¾?????å·?????å·???????è¾?????????????????????? */
...
void display(void)
{
    ...
    /* ????????? */
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); /* ???????????è¾?????????????? */
    glMatrixMode(GL_MODELVIEW);                         /* è¾???????????????????????è¾?????????????????????????????????????????????????? */
    glLoadIdentity();                                   /* è¾???????????????????????????????? */
    ...
    if(cameraflag){
       ...
       gluLookAt(CameraX2, CameraY2, CameraZ2, /* ?????è¾??????????????è¾?????????????? */
        player[clientID].pos.x , player[clientID].pos.y,player[clientID].pos.z ,        /* è¾????????????????è¾???????????è¾?????????????? */
        0, 0.5*cos(player[clientID].turn2), 0.0);       
    }
    else{
       
    gluLookAt(CameraX, CameraY, CameraZ, /* ?????è¾??????????????è¾?????????????? */
        player[clientID].pos.x, player[clientID].pos.y,player[clientID].pos.z,        /* è¾????????????????è¾???????????è¾?????????????? */
        0, 0.5*cos(player[clientID].turn2), 0.0);         /* ?????è¾???????????è¾???????????????????????????????? */
    }
           /* ?????è¾???????????è¾???????????????????????????????? */
    /*if(startflag == true){
        glPushMatrix();
    ...
    move_bullet(bullet_Num);    // ??????????????ç·???????????????è¾?????è¾?????????????
    draw_bullet(bullet_Num);    // ??????????????ç·???????????????????
...
void joystick(unsigned int buttonMask, int x, int y, int z)
{
        ...
        switch (buttonMask) {
               ...
               break;
                ...
                break;
                case GLUT_JOYSTICK_BUTTON_E:/*5???????????ç·??????*/
                 key7 = true;
                break;
                case GLUT_JOYSTICK_BUTTON_F:/*6???????????ç·??????*/
                 key7 = true;
                break;
                case GLUT_JOYSTICK_BUTTON_G:/*5???????????ç·??????*/
                can_special = true;
                    useSpecial();
                break;
                case GLUT_JOYSTICK_BUTTON_H:/*5???????????ç·??????*/
                can_special = true;
                    useSpecial();
                break;
...
void myInit(char *windowTitle)
{
    ...
    /* OpenGL?????????è¾??????????????è¾???????????????????????????????? */
    glutInitWindowPosition(0, 0);            /* ?????????è¾?????????????????????????????? */
    glutInitWindowSize(winWidth, winHeight); /* ?????????è¾????????????????è¾??????????????? */
    // glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH);   /* ?????è¾??????????????????? */

    glutCreateWindow(windowTitle);                      /* ?????????è¾????????????????????????? */
    ...
    glutDisplayFunc(display);    /* ????????è¾?????å·?????å·??????????? */
    glutTimerFunc(15, timer, 0); /* ??è¾??????????????????????15??????????è¾????????????????????? */
    createScoreBall();
    glutTimerFunc(15000, PopEnemy, 0);

    /* CG???????è¾?????å·?????å·???????è¾?????????? */
    glMatrixMode(GL_PROJECTION);             /* ?????????è¾??????????(????????????????å·?????????????)??????????????????????????????è¾??????????? */
    glLoadIdentity();                        /* ???????????????å·?????è¾?????????????????????? */
    gluPerspective(45.0, aspect, 1.0, 200.0); /* ???????????????å·?????è¾????????????????????? */
                                             /* ???????????45è¾??????????, ????è¾??????????è¾?????????? aspect?????????è¾?????????????????????????è¾?????????????? 1.0????????????????????????????è¾?????????????? 20.0 */
    glEnable(GL_DEPTH_TEST);                 /* ??????????????????????è¾????????????????? */
    glClearColor(0.658, 0.88, 0.925, 1.0);     
